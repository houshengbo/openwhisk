...
class WskRest() extends RunWskRestCmd {
    implicit val action = new WskRestAction
    implicit val trigger = new WskRestTrigger
    implicit val rule = new WskRestRule
    implicit val activation = new WskRestActivation
    implicit val pkg = new WskRestPackage
    implicit val namespace = new WskRestNamespace
    implicit val api = new WskRestApi
}
    ...
    override def list(namespace: Option[String] = None,
        ...
        if (expectedExitCode != DONTCARE_EXIT)
            if (r.statusCode.intValue != expectedExitCode)
                r.exitCode shouldBe expectedExitCode
        r
    }
...
class WskRestAction
    ...
    override def create(
        ...
        var bodyContent = JsObject("name" -> name.toJson, "namespace" -> namespace.toJson)

        if (update) {
            if ((kind != None) && ((inputKindType == "sequence") || (inputKindType == "native"))) {
                bodyContent = JsObject(bodyContent.fields + ("exec" -> JsObject(exec).toJson))
            }
            ...
            bodyContent = JsObject(
                bodyContent.fields +
                    ("exec" -> JsObject(exec).toJson,
                        "parameters" -> params,
                        "annotations" -> annos))

            bodyContent = timeout map { t =>
                JsObject(bodyContent.fields + ("limits" -> JsObject("timeout" -> t.toMillis.toJson)))
            } getOrElse bodyContent
        }
...
class WskRestApi extends RunWskRestCmd with BaseApi {
    ...
    override def create(basepath: Option[String] = None,
        ...
        val (ns, actionName) = this.getNamespaceActionName(action map { a =>
            a
        } getOrElse "")
...
class RunWskRestCmd() extends FlatSpec with RunWskCmd with Matchers with ScalaFutures with WskActorSystem {
    ...
    def deleteEntity(path: Path, whiskUrl: Uri = whiskRestUrl)(implicit wp: WskProps): Future[HttpResponse] = {
        val creds = getBasicHttpCredentials(wp)
        request(DELETE, whiskUrl.withPath(path), creds = creds)
    }
